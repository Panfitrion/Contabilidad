...
function setStore(key, val){ localStorage.setItem(key, JSON.stringify(val)); autoBackupSchedule(key); }
function getObj(key){ try{ return JSON.parse(localStorage.getItem(key)) || {}; }catch(e){ return {}; } }
function setObj(key, val){ localStorage.setItem(key, JSON.stringify(val)); autoBackupSchedule(key); }
