...
function generateInsights(){
  ...
  if(totalIngresos > 0){
    ...
    if(pctCompras > 60){
      insights.push({
        icon: '🛒', type: 'negative', severity: 'high',
        label: 'Compras Elevadas',
        text: 'Las compras a proveedores representan el <strong>'+pctCompras.toFixed(0)+'%</strong> de tus ingresos. Lo ideal es menos del 50%. Revisa precios o volúmenes de compra.',
        metric: pctCompras.toFixed(0)+'%', metricClass: 'negative',
        detail: 'Ideal: menos del 50%'
      });
    }
  }
...
function renderNominas(container){
  ...
  document.querySelectorAll('.svc-input').forEach(function(inp){
    inp.addEventListener('input', function(){
      var totalSvc = 0;
      document.querySelectorAll('.svc-input').forEach(function(i){ totalSvc += (parseFloat(i.value)||0); });
      var el = document.getElementById('svc-total');
      if(el) el.textContent = fmt(totalSvc);
    });
  });
...
function renderBaseDatos(container){
  ...
  if(bdSubTab === 'empleados') bdSubTab = 'servicios';
  switch(bdSubTab){
...
function autoBackupSchedule(key){
  ...
  _autoBackupTimer=setTimeout(function(){autoBackupRun();},30000);
  autoBackupIndicator('pending');
}
async function autoBackupRun(){
  if(_autoBackupInProgress||!_autoBackupDirty)return;
  var token=localStorage.getItem('_gh_token'),gistId=localStorage.getItem('_gh_gist_id');
  if(!token||!gistId)return;
  _autoBackupInProgress=true;autoBackupIndicator('syncing');
  ...
  payload.files[GIST_FILENAME]={content:JSON.stringify({saved_at:new Date().toISOString(),auto:true,data:data},null,2)};
  try{
    var res=await fetch('https://api.github.com/gists/'+gistId,{method:'PATCH',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(payload)});
    if(res.ok){_autoBackupDirty=false;localStorage.setItem('_last_auto_backup',new Date().toISOString());autoBackupIndicator('ok');}
    else autoBackupIndicator('error');
  }catch(e){autoBackupIndicator('error');}
  _autoBackupInProgress=false;
}
function autoBackupIndicator(state){
  var el=document.getElementById('auto-backup-dot');if(!el)return;
  var c={pending:'var(--warning)',syncing:'var(--primary)',ok:'var(--success)',error:'var(--danger)',idle:'var(--border)'};
  el.style.background=c[state]||c.idle;
  el.style.animation=(state==='pending'||state==='syncing')?'statusPulse 1.5s ease infinite':'none';
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  setTimeout(function(){
    var token=localStorage.getItem('_gh_token'),gistId=localStorage.getItem('_gh_gist_id');
    autoBackupIndicator(token&&gistId?'ok':'idle');
  },2000);
});
function showGistModal(mode){
  ...
  m.innerHTML='<div style="background:var(--surface);border-radius:var(--radius-lg);padding:28px;width:100%;max-width:440px;box-shadow:var(--shadow-lg)">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px"><div style="font-weight:700;font-size:1rem;color:var(--text)">'+(isB?'☁️ Backup':'☁️ Restaurar')+'</div>'+
    ...
    '<div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border-light);font-size:.72rem;color:var(--text-muted)">🔑 <a href="https://github.com/settings/tokens/new?scopes=gist&description=PanControl" target="_blank" style="color:var(--primary)">Crear token</a> — solo scope <strong>gist</strong></div></div>';
...
async function gistUpload(){
  ...
  try{
    ...
    localStorage.setItem('_last_auto_backup',new Date().toISOString());_autoBackupDirty=false;autoBackupIndicator('ok');
    var inp=document.getElementById('gist-id-input');if(inp)inp.value=json.id;
    gistSS('✓ Backup subido. ID: '+json.id,'ok');notify('Backup subido','success');
...
async function gistRestore(){
  ...
  try{
    ...
    var json=await res.json();if(!res.ok)throw new Error(json.message||res.statusText);
    var file=json.files[GIST_FILENAME];if(!file)throw new Error('No se encontró archivo');
    ...
    confirmDialog('¿Restaurar backup del '+savedAt+'?',function(ok){
      ...
      renderTab();notify('Datos restaurados','success');document.getElementById('gist-modal').remove();
    });
  }catch(e){gistSS('✗ '+e.message,'error');}
  gistSL(false);
}
...
function scanOCR(mode){
  ...
  m.innerHTML='<div style="background:var(--surface);border-radius:var(--radius-lg);padding:24px;width:100%;max-width:520px;max-height:92vh;overflow-y:auto;box-shadow:var(--shadow-lg)">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'+
      ...
      '<div style="font-size:.72rem;color:var(--text-muted);margin-top:2px">OCR local · sin internet</div></div>'+
    ...
    '<div id="ocr-progress" style="display:none;margin-bottom:14px"><div style="font-size:.8rem;color:var(--text-muted);margin-bottom:6px" id="ocr-plabel">Iniciando...</div>'+
      '<div style="background:var(--bg-alt);border-radius:4px;height:6px;overflow:hidden"><div id="ocr-pbar" style="height:100%;background:var(--primary);border-radius:4px;width:0%;transition:width .2s"></div></div></div>'+
...
async function ocrRun(file,mode){
  ...
  try{
    ...
    if(resEl)resEl.style.display='block';ocrParse(text,mode);
  }catch(err){
    if(prog)prog.style.display='none';
...
function ocrParse(text,mode){
  ...
  if(mode==='compra'){
    ...
    fieldsEl.innerHTML='<div style="display:grid;gap:10px"><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Proveedor</label><select id="ocr-prov" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"><option value="">Seleccionar...</option>'+provOpts+'</select>'+(!matchedProv?'<div style="font-size:.72rem;color:var(--warning);margin-top:3px">⚠ No detectado</div>':'')+'</div>'+
    ...
    fieldsEl.innerHTML='<div style="display:grid;gap:10px"><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Fecha</label><input id="ocr-fecha" type="date" value="'+(fecha||todayStr())+'" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Efectivo</label><input id="ocr-ef" type="number" value="'+(ef||'')+'" placeholder="0.00" step="0.01" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div>'+
...
function ocrApply(mode){
  if(mode==='compra'){
    ...
    if(!provId){notify('Selecciona un proveedor','warning');return;}
    if(total<=0){notify('Introduce el total','warning');return;}
    ...
    setStore('compras',compras);document.getElementById('ocr-modal').remove();renderTab();notify('Compra registrada: '+fmt(total),'success');
  }else{
    var fecha=document.getElementById('ocr-fecha')&&document.getElementById('ocr-fecha').value;
    ...
    setStore('ingresos',ingresos);document.getElementById('ocr-modal').remove();renderTab();notify('Ingreso registrado: '+fmt(ef+tj),'success');
  }
}
...
function importarPdfCuenta(){
  ...
  m.innerHTML='<div style="background:var(--surface);border-radius:var(--radius-lg);padding:24px;width:100%;max-width:560px;max-height:92vh;overflow-y:auto;box-shadow:var(--shadow-lg)">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div><div style="font-weight:700;font-size:1rem;color:var(--text)">📄 Importar PDFs de Pedidos</div>'+
    ...
    '<label id="pdf-dropzone" style="display:flex;flex-direction:column;align-items:center;gap:8px;padding:28px;border:2px dashed var(--border);border-radius:12px;cursor:pointer;background:var(--bg-alt);text-align:center;margin-bottom:14px" ondragover="event.preventDefault();this.style.borderColor=\'var(--primary)\'" ondragleave="this.style.borderColor=\'var(--border)\'" ondrop="pdfDrop(event)">'+
      '<span style="font-size:2rem">📄</span><span style="font-size:.88rem;font-weight:600;color:var(--text)">Selecciona uno o varios PDFs</span>'+
      '<span style="font-size:.75rem;color:var(--text-muted)">O arrastra aquí</span>'+
    ...
    '<div id="pdf-list"></div>'+
    '<div id="pdf-footer" style="display:none;margin-top:14px"><div id="pdf-summary" style="font-size:.82rem;color:var(--text-muted);margin-bottom:10px"></div>'+
      '<div style="display:flex;gap:8px"><button onclick="document.getElementById(\'pdf-modal\').remove()" style="flex:1;padding:10px;border:1px solid var(--border);border-radius:8px;background:none;cursor:pointer;font-size:.85rem;font-weight:600;color:var(--text-secondary)">Cancelar</button>'+
      '<button id="pdf-save-all-btn" onclick="pdfGuardarTodos()" style="flex:2;padding:10px;border:none;border-radius:8px;background:var(--success);color:#fff;cursor:pointer;font-size:.9rem;font-weight:600">✓ Crear cuentas</button></div></div></div>';
  document.body.appendChild(m);m.addEventListener('click',function(e){if(e.target===m)m.remove();});
}
...
async function pdfHandleFiles(files){
  _pdfResultados=[];var list=document.getElementById('pdf-list'),footer=document.getElementById('pdf-footer');if(!list)return;
  ...
  list.innerHTML=arr.map(function(f,i){return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg-alt);border-radius:8px;margin-bottom:8px;font-size:.83rem"><span>📄</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+f.name+'</span><span id="pdf-status-'+i+'" style="color:var(--text-muted);white-space:nowrap">⏳</span></div>';}).join('');
  for(var i=0;i<arr.length;i++){
    ...
    }catch(err){_pdfResultados.push({filename:file.name,data:null,error:err.message});if(statusEl)statusEl.innerHTML='<span style="color:var(--danger)">✗</span>';}
  }
  var ok=_pdfResultados.filter(function(r){return r.data&&r.data.monto&&r.data.cafeId;}).length;
  var sum=document.getElementById('pdf-summary');if(sum)sum.textContent=ok+' de '+arr.length+' PDFs listos.';
...
function pdfGuardarTodos(){
  ...
  _pdfResultados.forEach(function(r){
    if(!r.data||!r.data.monto)return;var d=r.data;
  ...
  setStore('cuentas_cobrar',cuentas);document.getElementById('pdf-modal').remove();
  if(lastMonth&&lastMonth!==selectedMonth){selectedMonth=lastMonth;updateMonthLabel();}
  renderTab();notify('✓ '+guardados+' cuenta'+(guardados!==1?'s':'')+' creada'+(guardados!==1?'s':''),'success');
}
