...
function renderCuentas(container){
  ...
  // Add cuenta form
  html += '<div class="section"><div class="section-header"><div><div class="section-title">Agregar Cuenta por Cobrar</div><div class="section-subtitle">Selecciona la cafetería e ingresa el total de la cuenta</div></div>' +
  ...
  // Pending — agrupadas por cafetería
  ...
  if(cuentas.length === 0){
    ...
    cuentas.forEach(function(c){
      if(!grupos[c.cafeId]){
        grupos[c.cafeId] = { cafeName: c.cafeName, cafeId: c.cafeId, cuentas: [], totalGrupo: 0 };
        ordenGrupos.push(c.cafeId);
      }
      grupos[c.cafeId].cuentas.push(c);
      grupos[c.cafeId].totalGrupo += parseFloat(c.monto)||0;
    });

    ordenGrupos.forEach(function(cafeId){
      var grupo = grupos[cafeId];
      var maxDias = 0;
      grupo.cuentas.forEach(function(c){
        var d = Math.floor((new Date() - new Date(c.fecha+'T12:00:00')) / 86400000);
        if(d > maxDias) maxDias = d;
      });
      var headerColor = maxDias > 14 ? 'var(--danger)' : maxDias > 7 ? 'var(--warning)' : 'var(--success)';
      var headerBg    = maxDias > 14 ? 'var(--danger-light)' : maxDias > 7 ? 'var(--warning-light)' : 'var(--success-light)';

      html += '<div class="card mb-12" style="overflow:hidden">';

      // Cabecera del grupo — cafetería + total
      html +=
        '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 18px;background:'+headerBg+';border-bottom:1px solid var(--border)">'+
          '<div style="display:flex;align-items:center;gap:10px">'+
            '<div style="width:32px;height:32px;border-radius:8px;background:'+headerColor+';display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:.82rem;flex-shrink:0">'+
              grupo.cafeName.substring(0,2).toUpperCase()+
            '</div>'+
            '<div>'+
              '<div style="font-weight:700;font-size:.92rem;color:var(--text)">'+grupo.cafeName+'</div>'+
              '<div style="font-size:.72rem;color:var(--text-muted)">'+grupo.cuentas.length+' cuenta'+(grupo.cuentas.length!==1?'s':'')+' pendiente'+(grupo.cuentas.length!==1?'s':'')+'</div>'+
            '</div>'+
          '</div>'+
          '<div style="text-align:right">'+
            '<div style="font-weight:800;font-size:1.05rem;color:'+headerColor+'">'+fmt(grupo.totalGrupo)+'</div>'+
            '<div style="font-size:.7rem;color:var(--text-muted)">total pendiente</div>'+
          '</div>'+
        '</div>';

      // Filas de cada cuenta dentro del grupo
      grupo.cuentas.forEach(function(c, idx){
        var diasPendiente = Math.floor((new Date() - new Date(c.fecha+'T12:00:00')) / 86400000);
        var dColor = diasPendiente > 14 ? 'var(--danger)' : diasPendiente > 7 ? 'var(--warning)' : 'var(--text-muted)';
        var borderTop = idx > 0 ? 'border-top:1px solid var(--border-light);' : '';
        html +=
          '<div style="display:flex;align-items:center;gap:12px;padding:11px 18px;'+borderTop+'transition:background .15s" onmouseover="this.style.background=\'var(--bg-alt)\'" onmouseout="this.style.background=\'\'">'+
            // Periodo
            ...
            // Días
            '<div style="font-size:.75rem;font-weight:600;color:'+dColor+';min-width:40px;text-align:center">'+diasPendiente+'d</div>'+
            // Acciones
            '<div style="display:flex;gap:6px;flex-shrink:0">'+
              '<button class="btn btn-success btn-sm" onclick="cobrarCuenta(\''+c.id+'\')" title="Marcar como cobrado" style="padding:5px 10px;font-size:.75rem">✓ Cobrar</button>'+
              '<button class="btn btn-ghost btn-sm btn-icon" onclick="deleteCuenta(\''+c.id+'\')" title="Eliminar" style="color:var(--danger);padding:5px 8px">'+
...
function saveCompra(){
  ...
  if(items.length === 0){
    var manualTotal=parseFloat((document.getElementById('comp-manual-total')||{}).value)||0;
    if(manualTotal<=0){
      if(!document.getElementById('comp-manual-total-wrap')){
        var wrap=document.createElement('div');wrap.id='comp-manual-total-wrap';
        wrap.style.cssText='margin-top:12px;padding:12px;background:var(--warning-light);border-radius:8px;border:1px solid var(--warning-lighter)';
        wrap.innerHTML='<div style="font-size:.78rem;font-weight:600;color:var(--warning);margin-bottom:8px">⚠ Sin productos. Ingresa el total:</div>'+
          '<div style="display:flex;gap:8px;align-items:center"><input id="comp-manual-total" type="number" min="0" step="0.01" placeholder="Total $0.00" style="flex:1;padding:8px 12px;border:1px solid var(--border);border-radius:8px;font-size:.9rem;color:var(--text);background:var(--surface);outline:none">'+
          '<button onclick="saveCompra()" style="padding:8px 16px;border:none;border-radius:8px;background:var(--primary);color:#fff;font-size:.85rem;font-weight:600;cursor:pointer">Guardar</button></div>';
        var pd=document.getElementById('comp-products');if(pd)pd.appendChild(wrap);
        setTimeout(function(){var el=document.getElementById('comp-manual-total');if(el)el.focus();},50);
      }
      notify('Ingresa el total de la factura','warning'); return;
    }
    items=[{productId:'manual',name:'Compra sin desglose',price:manualTotal,qty:1}];total=manualTotal;
  }
...
var _autoBackupTimer=null,_autoBackupDirty=false,_autoBackupInProgress=false;
var AUTO_BACKUP_KEYS=['pedidos','cuentas_cobrar','pagos_recibidos','ingresos','compras','servicios','rentas','colchon'];
function autoBackupSchedule(key){if(!AUTO_BACKUP_KEYS.includes(key))return;if(!localStorage.getItem('_gh_token')||!localStorage.getItem('_gh_gist_id'))return;_autoBackupDirty=true;if(_autoBackupTimer)clearTimeout(_autoBackupTimer);_autoBackupTimer=setTimeout(autoBackupRun,30000);autoBackupIndicator('pending');}
async function autoBackupRun(){if(_autoBackupInProgress||!_autoBackupDirty)return;var t=localStorage.getItem('_gh_token'),g=localStorage.getItem('_gh_gist_id');if(!t||!g)return;_autoBackupInProgress=true;autoBackupIndicator('syncing');var d={};GIST_KEYS.forEach(function(k){d[k]=localStorage.getItem(k);});var p={description:'PanControl auto-backup '+new Date().toISOString(),public:false,files:{}};p.files[GIST_FILENAME]={content:JSON.stringify({saved_at:new Date().toISOString(),auto:true,data:d},null,2)};try{var r=await fetch('https://api.github.com/gists/'+g,{method:'PATCH',headers:{Authorization:'Bearer '+t,'Content-Type':'application/json'},body:JSON.stringify(p)});if(r.ok){_autoBackupDirty=false;localStorage.setItem('_last_auto_backup',new Date().toISOString());autoBackupIndicator('ok');}else autoBackupIndicator('error');}catch(e){autoBackupIndicator('error');}_autoBackupInProgress=false;}
function autoBackupIndicator(s){var el=document.getElementById('auto-backup-dot');if(!el)return;var c={pending:'var(--warning)',syncing:'var(--primary)',ok:'var(--success)',error:'var(--danger)',idle:'var(--border)'};el.style.background=c[s]||c.idle;el.style.animation=(s==='pending'||s==='syncing')?'statusPulse 1.5s ease infinite':'none';}
window.addEventListener('beforeunload',function(){if(_autoBackupDirty&&!_autoBackupInProgress)autoBackupRun();});
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(_t&&_g&&!_d){var st=document.querySelector('.splash-text');if(st)st.textContent='Restaurando datos...';fetch('https://api.github.com/gists/'+_g,{headers:{Authorization:'Bearer '+_t}}).then(function(r){return r.json();}).then(function(j){var f=j.files&&j.files[GIST_FILENAME];if(!f)return;var c=JSON.parse(f.content),bd=c.data||{};GIST_KEYS.forEach(function(k){if(bd[k])localStorage.setItem(k,bd[k]);});notify('✓ Datos restaurados automáticamente','success');}).catch(function(){});}
  setTimeout(function(){autoBackupIndicator(localStorage.getItem('_gh_token')&&localStorage.getItem('_gh_gist_id')?'ok':'idle');},2000);
});
function showGistModal(mode){var ex=document.getElementById('gist-modal');if(ex)ex.remove();var token=localStorage.getItem('_gh_token')||'',gistId=localStorage.getItem('_gh_gist_id')||'',isB=mode==='backup';var m=document.createElement('div');m.id='gist-modal';m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(4px)';m.innerHTML='<div style="background:var(--surface);border-radius:var(--radius-lg);padding:28px;width:100%;max-width:440px;box-shadow:var(--shadow-lg)"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px"><div style="font-weight:700;font-size:1rem;color:var(--text)">'+(isB?'☁️ Backup':'☁️ Restaurar')+'</div><button onclick="document.getElementById(\'gist-modal\').remove()" style="border:none;background:none;cursor:pointer;color:var(--text-muted);font-size:1.4rem">×</button></div><div style="margin-bottom:14px"><label style="display:block;font-size:.78rem;font-weight:600;color:var(--text-secondary);margin-bottom:6px">GitHub Token</label><input id="gist-token-input" type="password" placeholder="ghp_xxxx" value="'+token+'" style="width:100%;padding:9px 12px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface);outline:none" oninput="localStorage.setItem(\'_gh_token\',this.value)"></div><div style="margin-bottom:20px"><label style="display:block;font-size:.78rem;font-weight:600;color:var(--text-secondary);margin-bottom:6px">Gist ID '+(isB?'(vacío = crear nuevo)':'')+'</label><input id="gist-id-input" type="text" placeholder="abc123..." value="'+gistId+'" style="width:100%;padding:9px 12px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface);outline:none" oninput="localStorage.setItem(\'_gh_gist_id\',this.value)"></div><div style="display:flex;gap:10px"><button onclick="document.getElementById(\'gist-modal\').remove()" style="flex:1;padding:10px;border:1px solid var(--border);border-radius:8px;background:none;cursor:pointer;font-size:.85rem;font-weight:600;color:var(--text-secondary)">Cancelar</button><button id="gist-action-btn" onclick="'+(isB?'gistUpload()':'gistRestore()')+'" style="flex:2;padding:10px;border:none;border-radius:8px;background:'+(isB?'var(--primary)':'var(--warning)')+';color:#fff;cursor:pointer;font-size:.85rem;font-weight:600">'+(isB?'Subir backup':'Restaurar datos')+'</button></div><div id="gist-status" style="margin-top:14px;font-size:.8rem;display:none"></div><div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border-light);font-size:.72rem;color:var(--text-muted)">🔑 <a href="https://github.com/settings/tokens/new?scopes=gist&description=PanControl" target="_blank" style="color:var(--primary)">Crear token</a> — solo scope <strong>gist</strong></div></div>';document.body.appendChild(m);m.addEventListener('click',function(e){if(e.target===m)m.remove();});}
...
function gistSL(l){var b=document.getElementById('gist-action-btn');if(!b)return;b.disabled=l;if(l){b._o=b.textContent;b.textContent='⏳...';}else if(b._o)b.textContent=b._o;}
async function gistUpload(){var token=document.getElementById('gist-token-input').value.trim(),gistId=document.getElementById('gist-id-input').value.trim();if(!token){gistSS('⚠️ Token requerido','error');return;}var data={};GIST_KEYS.forEach(function(k){data[k]=localStorage.getItem(k);});var payload={description:'PanControl backup '+new Date().toISOString(),public:false,files:{}};payload.files[GIST_FILENAME]={content:JSON.stringify({saved_at:new Date().toISOString(),data:data},null,2)};gistSL(true);gistSS('Subiendo...','info');try{var res=await fetch(gistId?'https://api.github.com/gists/'+gistId:'https://api.github.com/gists',{method:gistId?'PATCH':'POST',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(payload)});var json=await res.json();if(!res.ok)throw new Error(json.message||res.statusText);localStorage.setItem('_gh_token',token);localStorage.setItem('_gh_gist_id',json.id);localStorage.setItem('_last_auto_backup',new Date().toISOString());_autoBackupDirty=false;autoBackupIndicator('ok');var inp=document.getElementById('gist-id-input');if(inp)inp.value=json.id;gistSS('✓ Backup subido. ID: '+json.id,'ok');notify('Backup subido','success');}catch(e){gistSS('✗ '+e.message,'error');}gistSL(false);}
async function gistRestore(){var token=document.getElementById('gist-token-input').value.trim(),gistId=document.getElementById('gist-id-input').value.trim();if(!token){gistSS('⚠️ Token requerido','error');return;}if(!gistId){gistSS('⚠️ Gist ID requerido','error');return;}gistSL(true);gistSS('Descargando...','info');try{var res=await fetch('https://api.github.com/gists/'+gistId,{headers:{Authorization:'Bearer '+token}});var json=await res.json();if(!res.ok)throw new Error(json.message||res.statusText);var file=json.files[GIST_FILENAME];if(!file)throw new Error('No se encontró archivo');var content=JSON.parse(file.content),savedAt=content.saved_at?new Date(content.saved_at).toLocaleString('es-MX'):'?';gistSS('✓ Backup del '+savedAt,'ok');confirmDialog('¿Restaurar backup del '+savedAt+'?',function(ok){if(!ok){gistSL(false);return;}GIST_KEYS.forEach(function(k){if(content.data[k])localStorage.setItem(k,content.data[k]);});localStorage.setItem('_gh_token',token);localStorage.setItem('_gh_gist_id',gistId);renderTab();notify('Datos restaurados','success');document.getElementById('gist-modal').remove();});}catch(e){gistSS('✗ '+e.message,'error');}gistSL(false);}
var _ocrFile=null;
function scanOCR(mode){var ex=document.getElementById('ocr-modal');if(ex)ex.remove();_ocrFile=null;var m=document.createElement('div');m.id='ocr-modal';m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(4px)';m.innerHTML='<div style="background:var(--surface);border-radius:var(--radius-lg);padding:24px;width:100%;max-width:520px;max-height:92vh;overflow-y:auto;box-shadow:var(--shadow-lg)"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px"><div><div style="font-weight:700;font-size:1rem;color:var(--text)">📷 Escanear Factura</div><div style="font-size:.72rem;color:var(--text-muted);margin-top:2px">OCR local · sin internet</div></div><button onclick="document.getElementById(\'ocr-modal\').remove()" style="border:none;background:none;cursor:pointer;color:var(--text-muted);font-size:1.4rem;flex-shrink:0">×</button></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px"><label style="display:flex;flex-direction:column;align-items:center;gap:6px;padding:20px;border:2px dashed var(--border);border-radius:12px;cursor:pointer;background:var(--bg-alt);text-align:center"><span style="font-size:1.8rem">📷</span><span style="font-size:.82rem;font-weight:600;color:var(--text)">Cámara</span><input type="file" accept="image/*" capture="environment" style="display:none" onchange="ocrHandleFile(event,\''+mode+'\')"></label><label style="display:flex;flex-direction:column;align-items:center;gap:6px;padding:20px;border:2px dashed var(--border);border-radius:12px;cursor:pointer;background:var(--bg-alt);text-align:center"><span style="font-size:1.8rem">🖼</span><span style="font-size:.82rem;font-weight:600;color:var(--text)">Galería</span><input type="file" accept="image/*" style="display:none" onchange="ocrHandleFile(event,\''+mode+'\')"></label></div><div id="ocr-preview" style="display:none;margin-bottom:14px;text-align:center"><img id="ocr-img" style="max-width:100%;max-height:160px;border-radius:8px;border:1px solid var(--border)"></div><div id="ocr-progress" style="display:none;margin-bottom:14px"><div style="font-size:.8rem;color:var(--text-muted);margin-bottom:6px" id="ocr-plabel">Iniciando...</div><div style="background:var(--bg-alt);border-radius:4px;height:6px;overflow:hidden"><div id="ocr-pbar" style="height:100%;background:var(--primary);border-radius:4px;width:0%;transition:width .2s"></div></div></div><div id="ocr-result" style="display:none"><div style="font-size:.75rem;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Texto detectado</div><textarea id="ocr-raw" style="width:100%;height:80px;padding:8px;border:1px solid var(--border);border-radius:8px;font-size:.73rem;font-family:monospace;color:var(--text);background:var(--bg-alt);resize:vertical;outline:none"></textarea><div style="font-size:.75rem;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.06em;margin:10px 0 6px">Datos extraídos</div><div id="ocr-fields"></div><div style="display:flex;gap:8px;margin-top:14px"><button onclick="document.getElementById(\'ocr-modal\').remove()" style="flex:1;padding:10px;border:1px solid var(--border);border-radius:8px;background:none;cursor:pointer;font-size:.85rem;font-weight:600;color:var(--text-secondary)">Cancelar</button><button onclick="ocrApply(\''+mode+'\')" style="flex:2;padding:10px;border:none;border-radius:8px;background:var(--success);color:#fff;cursor:pointer;font-size:.9rem;font-weight:600">✓ Aplicar y guardar</button></div></div></div>';document.body.appendChild(m);m.addEventListener('click',function(e){if(e.target===m)m.remove();});}
...
async function ocrRun(file,mode){var prog=document.getElementById('ocr-progress'),plabel=document.getElementById('ocr-plabel'),pbar=document.getElementById('ocr-pbar');if(prog)prog.style.display='block';try{var worker=await Tesseract.createWorker('spa',1,{logger:function(msg){if(msg.status==='recognizing text'&&pbar&&plabel){var pct=Math.round((msg.progress||0)*100);pbar.style.width=pct+'%';plabel.textContent='Reconociendo... '+pct+'%';}}});var result=await worker.recognize(file);await worker.terminate();if(prog)prog.style.display='none';var text=(result.data&&result.data.text)||'';var rawEl=document.getElementById('ocr-raw'),resEl=document.getElementById('ocr-result');if(rawEl){rawEl.value=text;rawEl.oninput=function(){ocrParse(this.value,mode);};}if(resEl)resEl.style.display='block';ocrParse(text,mode);}catch(err){if(prog)prog.style.display='none';var resEl=document.getElementById('ocr-result');if(resEl){resEl.style.display='block';resEl.innerHTML='<div style="padding:12px;background:var(--danger-light);border-radius:8px;color:var(--danger);font-size:.85rem;margin-bottom:12px">✗ OCR falló</div><div id="ocr-fields"></div><div style="display:flex;gap:8px;margin-top:12px"><button onclick="document.getElementById(\'ocr-modal\').remove()" style="flex:1;padding:10px;border:1px solid var(--border);border-radius:8px;background:none;cursor:pointer;font-size:.85rem;font-weight:600;color:var(--text-secondary)">Cancelar</button><button onclick="ocrApply(\''+mode+'\')" style="flex:2;padding:10px;border:none;border-radius:8px;background:var(--success);color:#fff;cursor:pointer;font-size:.9rem;font-weight:600">✓ Aplicar</button></div>';ocrParse('',mode);}}}
...
function ocrParse(text,mode){var fieldsEl=document.getElementById('ocr-fields');if(!fieldsEl)return;var fecha=ocrExtractDate(text),total=ocrExtractTotal(text);if(mode==='compra'){var proveedores=getStore('proveedores'),matchedProv=null;var lines=text.split('\n').map(function(l){return l.trim();}).filter(Boolean);proveedores.forEach(function(p){lines.forEach(function(l){if(!matchedProv&&l.length>3&&(l.toLowerCase().includes(p.name.toLowerCase())||p.name.toLowerCase().includes(l.toLowerCase())))matchedProv=p;});});var provOpts=proveedores.map(function(p){return '<option value="'+p.id+'"'+(matchedProv&&p.id===matchedProv.id?' selected':'')+'>'+p.name+'</option>';}).join('');fieldsEl.innerHTML='<div style="display:grid;gap:10px"><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Proveedor</label><select id="ocr-prov" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"><option value="">Seleccionar...</option>'+provOpts+'</select>'+(!matchedProv?'<div style="font-size:.72rem;color:var(--warning);margin-top:3px">⚠ No detectado</div>':'')+'</div><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Fecha</label><input id="ocr-fecha" type="date" value="'+(fecha||todayStr())+'" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Total</label><input id="ocr-total" type="number" value="'+(total||'')+'" placeholder="0.00" step="0.01" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div></div>';}else{var efM=text.match(/efectivo[:\s]*\$?\s*([\d,]+\.?\d*)/i),tjM=text.match(/tarjeta[:\s]*\$?\s*([\d,]+\.?\d*)/i)||text.match(/cr[eé]dito[:\s]*\$?\s*([\d,]+\.?\d*)/i);var ef=efM?parseFloat(efM[1].replace(/,/g,'')):null,tj=tjM?parseFloat(tjM[1].replace(/,/g,'')):null;if(!ef&&!tj&&total)ef=total;fieldsEl.innerHTML='<div style="display:grid;gap:10px"><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Fecha</label><input id="ocr-fecha" type="date" value="'+(fecha||todayStr())+'" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Efectivo</label><input id="ocr-ef" type="number" value="'+(ef||'')+'" placeholder="0.00" step="0.01" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div><div><label style="display:block;font-size:.75rem;font-weight:600;color:var(--text-secondary);margin-bottom:4px">Tarjeta</label><input id="ocr-tj" type="number" value="'+(tj||'')+'" placeholder="0.00" step="0.01" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:.85rem;color:var(--text);background:var(--surface)"></div></div></div>';}}
function ocrApply(mode){if(mode==='compra'){var provId=document.getElementById('ocr-prov')&&document.getElementById('ocr-prov').value;var fecha=document.getElementById('ocr-fecha')&&document.getElementById('ocr-fecha').value;var total=parseFloat(document.getElementById('ocr-total')&&document.getElementById('ocr-total').value)||0;if(!provId){notify('Selecciona un proveedor','warning');return;}if(total<=0){notify('Introduce el total','warning');return;}var proveedores=getStore('proveedores'),prov=null;for(var i=0;i<proveedores.length;i++){if(proveedores[i].id===provId){prov=proveedores[i];break;}}var compras=getStore('compras');compras.push({id:uid(),provId:provId,provName:prov?prov.name:'',date:fecha||todayStr(),items:[{productId:'ocr',name:'Compra escaneada',price:total,qty:1}],total:total});setStore('compras',compras);document.getElementById('ocr-modal').remove();renderTab();notify('Compra registrada: '+fmt(total),'success');}else{var fecha=document.getElementById('ocr-fecha')&&document.getElementById('ocr-fecha').value;var ef=parseFloat(document.getElementById('ocr-ef')&&document.getElementById('ocr-ef').value)||0;var tj=parseFloat(document.getElementById('ocr-tj')&&document.getElementById('ocr-tj').value)||0;if(ef<=0&&tj<=0){notify('Ingresa al menos un monto','warning');return;}var ingresos=getStore('ingresos');ingresos.push({id:uid(),date:fecha||todayStr(),efectivo:ef,tarjeta:tj});setStore('ingresos',ingresos);document.getElementById('ocr-modal').remove();renderTab();notify('Ingreso registrado: '+fmt(ef+tj),'success');}}
function importarPdfCuenta(){var ex=document.getElementById('pdf-modal');if(ex)ex.remove();var m=document.createElement('div');m.id='pdf-modal';m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(4px)';m.innerHTML='<div style="background:var(--surface);border-radius:var(--radius-lg);padding:24px;width:100%;max-width:560px;max-height:92vh;overflow-y:auto;box-shadow:var(--shadow-lg)"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div><div style="font-weight:700;font-size:1rem;color:var(--text)">📄 Importar PDFs de Pedidos</div><div style="font-size:.72rem;color:var(--text-muted);margin-top:2px">Puedes subir varios a la vez</div></div><button onclick="document.getElementById(\'pdf-modal\').remove()" style="border:none;background:none;cursor:pointer;color:var(--text-muted);font-size:1.4rem">×</button></div><label style="display:flex;flex-direction:column;align-items:center;gap:8px;padding:28px;border:2px dashed var(--border);border-radius:12px;cursor:pointer;background:var(--bg-alt);text-align:center;margin-bottom:14px" ondragover="event.preventDefault();this.style.borderColor=\'var(--primary)\'" ondragleave="this.style.borderColor=\'var(--border)\'" ondrop="pdfDrop(event)"><span style="font-size:2rem">📄</span><span style="font-size:.88rem;font-weight:600;color:var(--text)">Selecciona PDFs</span><span style="font-size:.75rem;color:var(--text-muted)">O arrastra aquí</span><input type="file" accept=".pdf,application/pdf" multiple style="display:none" onchange="pdfHandleFiles(this.files)"></label><div id="pdf-list"></div><div id="pdf-footer" style="display:none;margin-top:14px"><div id="pdf-summary" style="font-size:.82rem;color:var(--text-muted);margin-bottom:10px"></div><div style="display:flex;gap:8px"><button onclick="document.getElementById(\'pdf-modal\').remove()" style="flex:1;padding:10px;border:1px solid var(--border);border-radius:8px;background:none;cursor:pointer;font-size:.85rem;font-weight:600;color:var(--text-secondary)">Cancelar</button><button id="pdf-save-all-btn" onclick="pdfGuardarTodos()" style="flex:2;padding:10px;border:none;border-radius:8px;background:var(--success);color:#fff;cursor:pointer;font-size:.9rem;font-weight:600">✓ Crear cuentas</button></div></div></div>';document.body.appendChild(m);m.addEventListener('click',function(e){if(e.target===m)m.remove();});}
function pdfDrop(e){e.preventDefault();if(e.dataTransfer.files&&e.dataTransfer.files.length)pdfHandleFiles(e.dataTransfer.files);}
var _pdfResultados=[];
async function pdfHandleFiles(files){_pdfResultados=[];var list=document.getElementById('pdf-list'),footer=document.getElementById('pdf-footer');if(!list)return;if(typeof pdfjsLib==='undefined'){list.innerHTML='<div style="color:var(--danger);font-size:.85rem;padding:10px">✗ PDF.js no cargó</div>';return;}pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';var arr=Array.from(files).filter(function(f){return f.type==='application/pdf'||f.name.endsWith('.pdf');});if(!arr.length){list.innerHTML='<div style="color:var(--warning);font-size:.85rem;padding:10px">⚠ No se encontraron PDFs</div>';return;}list.innerHTML=arr.map(function(f,i){return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg-alt);border-radius:8px;margin-bottom:8px;font-size:.83rem"><span>📄</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+f.name+'</span><span id="pdf-status-'+i+'" style="color:var(--text-muted);white-space:nowrap">⏳</span></div>';}).join('');for(var i=0;i<arr.length;i++){var file=arr[i],statusEl=document.getElementById('pdf-status-'+i);try{var text=await pdfExtractText(file),data=parsePdfPanfitrion(text,file.name);_pdfResultados.push({filename:file.name,data:data,error:null});if(statusEl){if(data.monto&&data.cafeId)statusEl.innerHTML='<span style="color:var(--success)">✓ $'+data.monto.toLocaleString('es-MX',{minimumFractionDigits:2})+' · '+data.cafeName+'</span>';else if(data.monto)statusEl.innerHTML='<span style="color:var(--warning)">⚠ Sin cafetería</span>';else statusEl.innerHTML='<span style="color:var(--warning)">⚠ Sin total</span>';}}catch(err){_pdfResultados.push({filename:file.name,data:null,error:err.message});if(statusEl)statusEl.innerHTML='<span style="color:var(--danger)">✗</span>';}}var ok=_pdfResultados.filter(function(r){return r.data&&r.data.monto&&r.data.cafeId;}).length;var sum=document.getElementById('pdf-summary');if(sum)sum.textContent=ok+' de '+arr.length+' PDFs listos.';var btn=document.getElementById('pdf-save-all-btn');if(btn)btn.textContent='✓ Crear '+ok+' cuenta'+(ok!==1?'s':'')+' por cobrar';if(footer)footer.style.display='block';}
...
function parsePdfPanfitrion(text,filename){var result={cafeName:'',periodo:'',monto:null,fecha:todayStr(),cafeId:null};var cafeM=text.match(/(?:Juárez|Juarez|juarez|Dakota[^a-z]+)\s+([A-ZÁÉÍÓÚÑ][^\n]+?)\s+Periodo/i);if(!cafeM)cafeM=text.match(/([A-ZÁÉÍÓÚÑ][a-záéíóúñA-ZÁÉÍÓÚÑ\s]{2,25})\s+Periodo/);if(cafeM)result.cafeName=cafeM[1].trim();var perM=text.match(/Periodo[:\s]+([^\n]+)/i);if(perM)result.periodo=perM[1].trim();var totM=text.match(/Total\s+Neto\s+a\s+Pagar[:\s$]*\$?\s*([\d,]+\.?\d*)/i);if(!totM)totM=text.match(/Total[:\s$]*\$?\s*([\d,]+\.?\d*)/i);if(totM)result.monto=parseFloat(totM[1].replace(/,/g,''));if(!result.cafeName&&filename){var fnM=filename.match(/^([^_\-\.]+)/);if(fnM)result.cafeName=fnM[1];}if(!result.periodo&&filename){var fpM=filename.match(/(\d+-\w+_al_\d+-\w+)/i);if(fpM)result.periodo=fpM[1].replace(/_/g,' ');}var cafeterias=getStore('cafeterias');cafeterias.forEach(function(c){if(result.cafeName&&(c.name.toLowerCase().includes(result.cafeName.toLowerCase())||result.cafeName.toLowerCase().includes(c.name.toLowerCase()))){result.cafeId=c.id;result.cafeName=c.name;}});var fechaM=result.periodo&&result.periodo.match(/(\d+)-(\w+)/);if(fechaM){var months={ene:1,feb:2,mar:3,abr:4,may:5,jun:6,jul:7,ago:8,sep:9,oct:10,nov:11,dic:12};var mon=months[fechaM[2].toLowerCase().substring(0,3)];if(mon){var yr=new Date().getFullYear();result.fecha=yr+'-'+String(mon).padStart(2,'0')+'-'+String(parseInt(fechaM[1])).padStart(2,'0');}}return result;}
function pdfGuardarTodos(){var guardados=0,cuentas=getStore('cuentas_cobrar'),cafeterias=getStore('cafeterias'),lastMonth=null;_pdfResultados.forEach(function(r){if(!r.data||!r.data.monto)return;var d=r.data;if(!d.cafeId&&d.cafeName){cafeterias.forEach(function(c){if(c.name.toLowerCase().includes(d.cafeName.toLowerCase())||d.cafeName.toLowerCase().includes(c.name.toLowerCase())){d.cafeId=c.id;d.cafeName=c.name;}});}if(!d.cafeId)return;var parts=d.fecha.split('-'),month=parts[0]+'-'+parts[1];cuentas.push({id:uid(),cafeId:d.cafeId,cafeName:d.cafeName,periodo:d.periodo,monto:d.monto,fecha:d.fecha,month:month});lastMonth=month;guardados++;});setStore('cuentas_cobrar',cuentas);document.getElementById('pdf-modal').remove();if(lastMonth&&lastMonth!==selectedMonth){selectedMonth=lastMonth;updateMonthLabel();}renderTab();notify('✓ '+guardados+' cuenta'+(guardados!==1?'s':'')+' creada'+(guardados!==1?'s':''),'success');}
