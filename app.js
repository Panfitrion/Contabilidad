  ...
  if (items.length === 0) {
    ...
    if (manualTotal <= 0) {
      if (!document.getElementById('comp-manual-total-wrap')) {
        ...
        wrap.id = 'comp-manual-total-wrap';
        ...
        wrap.innerHTML = '<div style="font-size:.78rem;font-weight:600;color:var(--warning);margin-bottom:8px">⚠ Sin productos. Ingresa el total de la factura:</div>' +
          '<div style="display:flex;gap:8px;align-items:center">' +
          '<input id="comp-manual-total" type="number" min="0" step="0.01" placeholder="Total $0.00" style="flex:1;padding:8px 12px;border:1px solid var(--border);border-radius:8px;font-size:.9rem;color:var(--text);background:var(--surface);outline:none">' +
        ...
        var pd = document.getElementById('comp-products');
        if (pd) pd.appendChild(wrap);
...
var tutorialSections = [
  {
    ...
    steps: [
      ...
      { icon: '3️⃣', title: 'Crea pedidos semanales', desc: 'Ve a <strong>Pedidos Cafeterías</strong>, selecciona la cafetería y la fecha de inicio de la semana. El sistema generará automáticamente la tabla de lunes a sábado.' },
  ...
  },
  {
    ...
    icon: '☁️',
    steps: [
      { icon: '☁️', title: '¿Qué es?', desc: 'PanControl guarda todos tus datos en el navegador (<strong>localStorage</strong>). La sincronización en la nube te permite hacer un <strong>backup en GitHub Gist</strong> y restaurarlo en cualquier otro dispositivo (celular, tablet, otra computadora).' },
      { icon: '🔑', title: 'Crear tu token de GitHub', desc: 'Ve a <a href="https://github.com/settings/tokens/new?scopes=gist&description=PanControl" target="_blank" style="color:var(--primary)">github.com/settings/tokens/new</a>, ponle un nombre (ej. "PanControl"), marca <strong>solo el scope "gist"</strong> y genera el token. Cópialo — solo se muestra una vez.' },
      { icon: '⬆️', title: 'Hacer un backup (subir a la nube)', desc: 'En el Dashboard, presiona <strong>☁️ Nube</strong>. Introduce tu token de GitHub y deja el Gist ID vacío la primera vez. Al subir, se genera un Gist privado y el ID se guarda automáticamente.' },
      { icon: '⬇️', title: 'Restaurar en otro dispositivo', desc: 'En el otro dispositivo, abre la app y presiona <strong>☁️ Restaurar</strong>. Introduce el mismo token y el Gist ID. Todos tus datos se restauran al instante.' },
      { icon: '🤖', title: 'Restauración automática', desc: 'Una vez que introduces el token y Gist ID en un dispositivo, la próxima vez que abras la app <strong>restaura automáticamente</strong> en silencio si no hay datos locales. El splash dirá "Restaurando datos...".' }
    ],
    tip: { type: 'warning', icon: '🔒', text: 'El token se guarda en tu dispositivo. Usa un token con scope "gist" únicamente — así, aunque alguien lo vea, solo puede acceder a tus Gists, nada más de tu cuenta GitHub.' },
    goTo: 'dashboard',
  ...
  },
  {
    ...
    icon: '📷',
    steps: [
      { icon: '📷', title: '¿Qué es el escáner OCR?', desc: 'En las secciones de <strong>Compras de Proveedores</strong> e <strong>Ingresos</strong>, hay un botón <strong>"📷 Escanear"</strong>. Al usarlo, puedes tomar una foto de la factura y el sistema extrae automáticamente los datos usando tecnología OCR (reconocimiento de texto).' },
      { icon: '🔒', title: 'Completamente local', desc: 'El OCR funciona <strong>sin internet</strong> y sin enviar datos a ningún servidor. Todo el procesamiento ocurre en tu propio dispositivo.' },
      { icon: '📱', title: 'Cómo usarlo', desc: 'Presiona <strong>"📷 Escanear"</strong> en Compras o Ingresos. Elige <strong>Cámara</strong> (foto en el momento) o <strong>Galería</strong> (imagen guardada). El sistema procesa la imagen y muestra los datos detectados para que los revises.' },
      { icon: '✏️', title: 'Revisa y corrige', desc: 'El OCR muestra el texto detectado y los campos extraídos (proveedor, fecha, total). <strong>Siempre revisa</strong> los datos antes de aplicar — puedes corregirlos directamente en los campos.' },
      { icon: '✓', title: 'Aplicar y guardar', desc: 'Para <strong>compras</strong>: detecta el proveedor y el total, y guarda directamente la compra. Para <strong>ingresos</strong>: detecta el efectivo, tarjeta y fecha, y registra el ingreso.' }
    ],
    tip: { type: 'warning', icon: '⚠️', text: 'El OCR funciona mejor con fotos bien iluminadas, papel plano y texto impreso. Si la calidad es baja, los campos quedan editables para captura manual.' },
    goTo: 'compras',
    goLabel: 'Ir a Compras'
  },
  {
    id: 'pdfcuentas',
    title: 'Importar PDFs de Pedidos',
    icon: '📄',
    steps: [
      { icon: '📄', title: '¿Qué hace?', desc: 'En <strong>Cuentas por Cobrar</strong> hay un botón <strong>"📄 Subir PDFs"</strong>. Sube los PDFs que genera PanControl para cada cafetería y el sistema crea automáticamente las cuentas por cobrar correspondientes.' },
      { icon: '📦', title: 'Múltiples a la vez', desc: 'Puedes subir <strong>varios PDFs al mismo tiempo</strong> — uno por cafetería. Arrástralos o selecciónlos con el selector de archivos. El sistema procesa todos en paralelo.' },
      { icon: '🔍', title: 'Datos extraídos', desc: 'De cada PDF extrae: <strong>nombre de la cafetería</strong> (busca coincidencia con las registradas), <strong>periodo</strong> (ej. "2-Mar al 7-Mar"), <strong>total a cobrar</strong> y la <strong>fecha</strong> inferida del periodo.' },
      { icon: '✅', title: 'Semáforo de estado', desc: 'Cada archivo muestra su estado: <strong>✓ verde</strong> = listo para guardar, <strong>⚠ amarillo</strong> = se detectó el monto pero no la cafetería, <strong>✗ rojo</strong> = error leyendo el PDF.' },
      { icon: '💾', title: 'Guardar todo', desc: 'Con un solo clic en <strong>"Crear X cuentas por cobrar"</strong>, guarda todas las que están en verde. Si el mes del PDF es diferente al seleccionado, cambia automáticamente al mes correcto.' }
    ],
    tip: { type: 'info', icon: '💡', text: 'Si el nombre de la cafetería no coincide, el sistema también intenta leerlo del nombre del archivo (ej. "Zuzu_2-Mar_al_7-Mar.pdf" → detecta "Zuzu").' },
    goTo: 'cuentas',
  ...
  },
  {
    ...
    icon: '🗄️',
    steps: [
      { icon: '🍞', title: 'Catálogo de Panes', desc: 'Registra todos los tipos de pan que produces. Solo necesitas el <strong>nombre</strong>. Los panes "Croissant" y "Chocolatín" se marcan automáticamente como <strong>devolvibles</strong>.' },
      ...
      { icon: '👥', title: 'Empleados', desc: 'Registra nombre y puesto de cada trabajador. Aparecerán en <strong>Nóminas</strong> para asignar sueldos.' },
      { icon: '⚡', title: 'Servicios Fijos', desc: 'Agrega los gastos recurrentes: Luz, Agua, Gas, Internet, etc. Aparecerán en <strong>Servicios</strong> para registrar el costo mensual.' }
    ],
  ...
  },
  {
    ...
    icon: '📋',
    steps: [
      { icon: '📌', title: 'Crear un pedido', desc: 'Selecciona la <strong>cafetería</strong> y la <strong>fecha de inicio</strong> de la semana. El sistema genera una tabla con los productos asignados y los días de lunes a sábado.' },
      { icon: '✏️', title: 'Registrar cantidades', desc: 'En cada celda escribe la <strong>cantidad de piezas</strong> entregadas ese día. Los totales se calculan <strong>en tiempo real</strong>.' },
      { icon: '↩️', title: 'Devoluciones', desc: 'Si la cafetería tiene devoluciones habilitadas, aparecen columnas para registrar piezas devueltas. El total neto se calcula restando las devoluciones.' },
      { icon: '📄', title: 'Exportar PDF', desc: 'Genera un PDF profesional con el detalle del pedido. Listo para imprimir o enviar al cliente por WhatsApp.' },
  ...
  },
  {
    ...
    icon: '💰',
    steps: [
      { icon: '📊', title: 'Vista general', desc: 'Muestra dos KPIs: el <strong>total pendiente</strong> de cobro y el <strong>total ya cobrado</strong> del mes.' },
      { icon: '📄', title: 'Importar desde PDF', desc: 'Usa el botón <strong>"📄 Subir PDFs"</strong> para importar automáticamente las cuentas desde los PDFs de pedidos. Puedes subir varios a la vez.' },
      { icon: '🏪', title: 'Tarjetas de cuentas', desc: 'Cada cuenta muestra: cafetería, periodo, monto y <strong>días de antigüedad</strong>. Más de 14 días se marca en rojo.' },
      { icon: '✅', title: 'Cobrar', desc: 'Al presionar "Cobrar", se registra el pago y aparece en el <strong>historial de pagos recibidos</strong>.' }
    ],
    flow: [
      'Se cierra un pedido → aparece como cuenta pendiente',
      'O subes el PDF → se crea la cuenta automáticamente',
      'La cuenta muestra monto y antigüedad',
      'Cuando la cafetería paga, presionas "Cobrar"',
      'Se mueve al historial de pagos del mes'
    ],
    tip: { type: 'success', icon: '✓', text: 'Las cuentas se generan automáticamente al cerrar un pedido o al importar un PDF. No necesitas crearlas manualmente.' },
    goTo: 'cuentas',
    goLabel: 'Ir a Cuentas por Cobrar'
  },
  {
    ...
    icon: '📈',
    steps: [
      { icon: '💵', title: 'Ventas al público', desc: 'Registra las ventas diarias en la panadería. Separa en <strong>efectivo</strong> y <strong>tarjeta</strong>.' },
      { icon: '📷', title: 'Escanear factura', desc: 'Usa el botón <strong>"📷 Escanear"</strong> para fotografiar una factura o nota de venta. El OCR extrae la fecha y los montos automáticamente.' },
    ...
    tip: { type: 'info', icon: '💡', text: 'Los ingresos de cafeterías NO se registran aquí — esos se controlan en Pedidos y Cuentas por Cobrar. Aquí solo van las ventas directas al público.' },
    goTo: 'ingresos',
  ...
  },
  {
    ...
    icon: '🛒',
    steps: [
      { icon: '📷', title: 'Escanear factura del proveedor', desc: 'Usa el botón <strong>"📷 Escanear"</strong> para fotografiar la factura. El OCR detecta el proveedor, fecha y total, y guarda la compra directamente.' },
      { icon: '🚛', title: 'Captura manual', desc: 'Selecciona el proveedor — se cargan sus productos automáticamente. Ingresa las cantidades y el total se calcula solo.' },
      { icon: '🔄', title: 'Repetir última compra', desc: 'Si sueles comprar lo mismo, usa <strong>"Repetir última compra"</strong> para cargar las cantidades de tu compra anterior.' },
      { icon: '💾', title: 'Sin desglose de productos', desc: 'Si no tienes los productos cargados, selecciona el proveedor, deja las cantidades vacías y guarda — el sistema pedirá el total de la factura.' }
    ],
    tip: { type: 'info', icon: '💡', text: 'El escáner OCR funciona mejor con buena iluminación. Si el reconocimiento falla, los campos quedan editables para captura manual.' },
    goTo: 'compras',
    goLabel: 'Ir a Compras'
  },
  {
    ...
    icon: '👥',
    steps: [
      { icon: '💼', title: 'Nómina', desc: 'Lista de todos los empleados. Junto a cada nombre hay un <strong>campo para ingresar su sueldo</strong> del mes. El total se actualiza en tiempo real.' },
      { icon: '⚡', title: 'Servicios', desc: 'Lista de servicios fijos (Luz, Agua, Gas, etc.). Ingresa el costo de cada uno para el mes.' },
      { icon: '💾', title: 'Guardar', desc: 'Presiona <strong>"Guardar Todo"</strong> o <kbd>Ctrl</kbd>+<kbd>S</kbd> para almacenar nómina y servicios del mes.' }
    ],
    tip: { type: 'warning', icon: '⚠️', text: 'Los montos se guardan por mes. Recuerda guardar antes de cambiar de mes.' },
    goTo: 'nominas',
  ...
  },
  {
    ...
    icon: '🏠',
    steps: [
      { icon: '🏠', title: 'Renta mensual', desc: 'La renta es de <strong>$46,600</strong> fijos. Ingresa cuánto pagarás en <strong>efectivo</strong> y el sistema calcula automáticamente lo que corresponde a <strong>tarjeta</strong>.' },
      { icon: '🛡️', title: 'Colchón financiero', desc: 'Reserva de dinero para imprevistos. Puedes <strong>agregar</strong> o <strong>retirar</strong> dinero. Los retiros requieren un concepto obligatorio.' },
      { icon: '📊', title: 'Saldo acumulado', desc: 'Muestra el saldo total del colchón (suma de todos los movimientos de todos los meses).' }
    ],
    tip: { type: 'info', icon: '💡', text: 'El colchón es acumulativo entre meses. Si depositas $5,000 en enero y $3,000 en febrero, tu saldo será $8,000 (menos retiros).' },
    goTo: 'renta',
  ...
  },
  {
    ...
    icon: '📊',
    steps: [
      { icon: '📊', title: 'KPIs principales', desc: 'Cuatro indicadores: <strong>Ingresos Totales</strong>, <strong>Gastos Totales</strong>, <strong>Utilidad</strong> y <strong>Salud Financiera</strong>. Incluye comparación con el mes anterior.' },
      { icon: '🍩', title: 'Gráficos', desc: 'Gráfico de dona con distribución de gastos y gráfico de barras comparando ingresos vs gastos vs utilidad.' },
      { icon: '💾', title: 'Backup local', desc: '<strong>"Backup local"</strong> descarga todos tus datos como archivo JSON al dispositivo.' },
      { icon: '☁️', title: 'Backup en la nube', desc: '<strong>"☁️ Nube"</strong> sube tus datos a GitHub Gist (privado). <strong>"☁️ Restaurar"</strong> los descarga en cualquier otro dispositivo.' },
      { icon: '📄', title: 'Exportar PDF', desc: 'Genera un reporte mensual en PDF con el resumen financiero completo.' }
    ],
    tip: { type: 'success', icon: '✓', text: 'El Dashboard se actualiza automáticamente con los datos de todas las secciones.' },
    goTo: 'dashboard',
    goLabel: 'Ir al Dashboard'
  },
  {
    ...
    steps: [
      ...
      { icon: '💾', title: 'Guardar', desc: '<kbd>Ctrl</kbd>+<kbd>S</kbd> guarda en la sección actual' },
      { icon: '📄', title: 'Exportar', desc: '<kbd>Alt</kbd>+<kbd>P</kbd> exporta el PDF disponible en la sección actual' },
      { icon: '🔄', title: 'Otros', desc: '<kbd>Alt</kbd>+<kbd>N</kbd> enfoca el primer input, <kbd>Alt</kbd>+<kbd>R</kbd> refresca, <kbd>?</kbd> muestra atajos, <kbd>Esc</kbd> cierra paneles' }
    ],
    tip: { type: 'info', icon: '⌨️', text: 'Presiona <kbd>?</kbd> en cualquier momento para ver el panel rápido de atajos.' }
  },
  {
    id: 'flujo',
    ...
    flow: [
      ...
      '<strong>Sábado:</strong> Exporta PDF del pedido → envíalo al cliente',
    ...
    extraFlow: [
      '<strong>Diario:</strong> Registra ventas al público en Ingresos (o escanea la nota)',
      '<strong>Cuando compres:</strong> Registra compras (escanea la factura o captura manual)',
      '<strong>Al recibir PDFs:</strong> Sube todos los PDFs en Cuentas por Cobrar de una vez',
      '<strong>Fin de mes:</strong> Ingresa nóminas y costos de servicios',
      '<strong>Fin de mes:</strong> Registra pago de renta y ajusta el colchón',
      '<strong>Fin de mes:</strong> Revisa el Dashboard y haz backup en la nube ☁️'
    ],
    tip: { type: 'success', icon: '✓', text: 'Siguiendo este flujo cada semana y mes, tendrás un control financiero completo de tu panadería sin esfuerzo extra.' }
  }
];
...
function showGistModal(mode) {
  ...
  m.innerHTML = '<div style="background:var(--surface);border-radius:var(--radius-lg);padding:28px;width:100%;max-width:440px;box-shadow:var(--shadow-lg)">' +
    ...
    '<button id="gist-action-btn" onclick="' + (isB ? 'gistUpload()' : 'gistRestore()') + '" style="flex:2;padding:10px;border:none;border-radius:8px;background:' + (isB ? 'var(--primary)' : 'var(--warning)') + ';color:#fff;cursor:pointer;font-size:.85rem;font-weight:600">' + (isB ? 'Subir backup' : 'Restaurar datos') + '</button></div>' +
...
async function gistRestore() {
  ...
  try {
    ...
    gistSS('✓ Backup del ' + savedAt, 'ok');
    confirmDialog('¿Restaurar backup del ' + savedAt + '? Reemplazará todos los datos actuales.', function (ok) {
...
// ===== IMPORTAR PDFs → CUENTAS POR COBRAR =====
