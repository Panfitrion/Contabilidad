...
function getMonthRange(monthInputVal) {
    if (!monthInputVal) {
        ...
        return {
            ...
            end: `${y}-${m.toString().padStart(2, '0')}-31` // Supabase maneja días extra sin error usualmente, o usar lógica real
        };
        // Mejor: calcular último día real
    }
    const [y, m] = monthInputVal.split('-');
    const lastDay = new Date(y, m, 0).getDate();
    return { start: `${monthInputVal}-01`, end: `${monthInputVal}-${lastDay}` };
}
...
async function resetPedidoView() {
    ...
    precios.forEach(p => {
        ...
        const isDev = cafe.devolucion && (n.includes("croissant") || n.includes("chocolat") || n.includes("chocolatín"));
...
async function exportarPedidoPDF() {
    ...
    // Usar tamaño de página A5 para un PDF más compacto
    ...
    // Ajustar tamaño y posición del logo para A5
    const logoAX = 10, logoAY = 8, logoAW = 35, logoAH = 22;
    try {
    ...
    doc.setFontSize(11);
    doc.text('atencion@panfitrion.com.mx', 74, 13, { align: 'center' });
    doc.text('+525521397371', 74, 19, { align: 'center' });
    doc.text('Dakota 410. Benito Juárez', 74, 25, { align: 'center' });

    // Cafetería nombre centrado arriba del periodo
    doc.setFontSize(15);

    doc.setFont('helvetica', 'bold');
    doc.text(cafe.nombre, 74, 32, { align: 'center' });

    // Periodo centrado debajo del nombre
    doc.setFontSize(13);

    doc.setFont('helvetica', 'bold');
    doc.text(`Periodo: ${rango.replace('_al_', ' al ')}`, 74, 39, { align: 'center' });
    ...
    // Encabezados: Pan, (días), Total
    const headers = Array.from(thead.querySelectorAll('th'))
        .filter((_, idx) => idx !== 1) // Omitir columna de precio unitario
        .map(th => th.innerText);

    // Filas: nombre, cantidades por día, total
    const rows = [];
    tbody.querySelectorAll('tr').forEach(tr => {
        const tds = Array.from(tr.querySelectorAll('td'));
        if (tds.length === 0) return;
        // Omitir columna de precio unitario (índice 1)
        const row = [];
        ...
        // En la última columna, poner la suma de cantidades
        row.push(sumaCant);
        rows.push(row);
    });
    ...
    doc.autoTable({
        head: [headers],
        body: rows,
        // Ajustar el inicio de la tabla para el nuevo tamaño
        startY: 48,
        theme: 'plain',
        styles: {
            font: 'helvetica',
        ...
        // No didParseCell: plain table, no custom cell coloring
    });
    // Total Neto
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.text('Total Neto:', 120, doc.lastAutoTable.finalY + 12, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.setTextColor('#222');
    doc.setFontSize(14);
    doc.text(document.getElementById('pedido-total-display').innerText, 180, doc.lastAutoTable.finalY + 12, { align: 'center' });
...
async function renderConfigTables() {
    ...
    const render = (id, data, table) => {
        ...
        if (el) el.innerHTML = data.map(i => `<tr><td>${i.nombre}</td><td><button onclick="deleteItem('${table}', ${i.id})">DEL</button></td></tr>`).join('');
    };
...
async function dbSaveCafeteria() {
    ...
    // 1. Guardar Cafetería
    const { error } = await supabase.from('cafeterias').insert([{ id: cafeId, nombre, devolucion }]);
    if (error) return alert("Error al crear cafetería: " + error.message);
    ...
    document.querySelectorAll('#table-lista-manual-cafe tr').forEach(row => {
        ...
        if (check && check.checked) {
            precios.push({
                cafeteria_id: cafeId,
                nombre_producto: check.dataset.nombre,
    ...
    if (precios.length > 0) {
        ...
        if (errP) alert("Advertencia: Cafetería creada pero error en precios: " + errP.message);
    }
...
async function prepareNew(t) {
    ...
    // Fallback refresh for lists if needed
    renderConfigTables();
}
...
async function initApp() {
    // 1. Iniciar App Directamente (Sin Login)

    renderMonthSelector(); // Populate month dropdown first
    navigate('dashboard');
...
function renderMonthSelector() {
    ...
    // Generar rango: Año pasado, este año, próximo año
    ...
    // Set current month
    const defaultVal = `${currentY}-${currentM.toString().padStart(2, '0')}`;
